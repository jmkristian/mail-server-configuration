...
this.uid)}).mouseup(function(a){return 1==a.which&&!b.drag_active&&!$(a.currentTarget).is(".ui-droppable-active")?b.click_row(a,this.uid):!0});if((bw.ie||bw.edge)&&bw.pointer)$(a).on("pointerdown",function(a){if("touch"==a.pointerType)return b.touch_start_time=(new Date).getTime(),!1}).on("pointerup",function(a){if("touch"==a.pointerType&&(new Date).getTime()-b.touch_start_time<=b.touch_event_time)return b.drag_row(a,this.uid),b.click_row(a,this.uid)});else bw.touch&&a.addEventListener&&(a.addEventListener("touchstart",
...
"rcmdraglayer").css({position:"absolute",display:"none","z-index":2E3}).appendTo(document.body);$(this.row_tagname()+".selected",this.tbody).each(function(){var a=d.get_row_uid(this),b=d.rows[a];if(b&&!(-1<$.inArray(a,c))&&(c.push(a),b.has_children&&!b.expanded&&$.each(d.row_children(a),function(){-1<$.inArray(this,c)||c.push(this)}),11<c.length))return!1});$.each(c,function(a,b){if(10<a)return d.draglayer.append($("<div>").text("...")),!1;var c=d.subject_column();$("> "+d.col_tagname(),d.rows[b].obj).each(function(a,
b){if(0>c||0<=c&&c==a){b=$(b).clone();$(b).find(".skip-on-drag").remove();var e=b.text();if(e)return e=$.trim(e),e=50<e.length?e.substring(0,50)+"...":e,d.draglayer.append($("<div>").text(e)),!1}})});this.draglayer.show();this.drag_active=!0;this.triggerEvent("dragstart")}this.drag_active&&this.draglayer&&(b=rcube_event.get_mouse_pos(a),this.draglayer.css({left:b.x+20+"px",top:b.y-5+(bw.ie?document.documentElement.scrollTop:0)+"px"}),this.triggerEvent("dragmove",a?a:window.event));return this.drag_start=
...
!this.list.rowcount&&b==this.cols.length&&(c-=2);$("#rcmcolumnindicator").css({width:c+"px"});this.triggerEvent("column_dragmove",a?a:window.event)}return this.drag_start=!1},column_drag_mouse_up:function(a){document.onmousemove=null;this.col_draglayer&&(this.col_draglayer.remove(),this.col_draglayer=null);rcube_event.remove_listener({event:"mousemove",object:this,method:"column_drag_mouse_move"});rcube_event.remove_listener({event:"mouseup",object:this,method:"column_drag_mouse_up"});this.del_dragfix();
if(this.col_drag_active&&(this.col_drag_active=!1,this.focus(),this.triggerEvent("column_dragend",a),null!==this.selected_column&&this.cols&&this.cols.length)){var b,c=0,d=rcube_event.get_mouse_pos(a);for(b=0;b<this.cols.length;b++)if(d.x>=this.cols[b]/2+this.list_pos+c)c+=this.cols[b];else break;b!=this.selected_column&&b!=this.selected_column+1&&this.column_replace(this.selected_column,b)}return rcube_event.cancel(a)},row_children:function(a){if(!this.rows[a]||!this.rows[a].has_children)return[];

...
function GoogieSpell(s,t,u){var q=this,r=rcmail.get_cookie("language");GOOGIE_CUR_LANG=null!=r?r:GOOGIE_DEFAULT_LANG;this.array_keys=function(a){var b=[],c;for(c in a)b.push([c]);return b};this.img_dir=s;this.server_url=t;this.lang_to_word=this.org_lang_to_word={da:"Dansk",de:"Deutsch",en:"English",es:"Espa\u00f1ol",fr:"Fran\u00e7ais",it:"Italiano",nl:"Nederlands",pl:"Polski",pt:"Portugu\u00eas",ru:"\u0420\u0443\u0441\u0441\u043a\u0438\u0439",fi:"Suomi",sv:"Svenska"};this.langlist_codes=this.array_keys(this.lang_to_word);
...
this.edit_layer_dbl_click=!0;this.custom_no_spelling_error=this.custom_ajax_error=null;this.extra_menu_items=[];this.custom_spellcheck_starter=null;this.main_controller=!0;this.has_dictionary=u;this.all_errors_fixed_observer=this.show_menu_observer=this.spelling_state_observer=this.lang_state_observer=null;this.use_focus=!1;this.focus_link_b=this.focus_link_t=null;this.cnt_errors_fixed=this.cnt_errors=0;$(document).click(function(a){"1"!=$(a.target).attr("googie_action_btn")&&q.isErrorWindowShown()&&
...
var b=this;$.ajax({type:"POST",url:this.getUrl(),data:this.createXMLReq(a),dataType:"text",error:function(a){b.custom_ajax_error?b.custom_ajax_error(b):rcmail.alert_dialog("An error was encountered on the server. Please try again later.");b.main_controller&&($(b.spell_span).remove(),b.removeIndicator());b.checkSpellingState()},success:function(a){b.processData(a);b.results.length||(b.custom_no_spelling_error?b.custom_no_spelling_error(b):b.flashNoSpellingErrorState());b.removeIndicator()}})};this.learnWord=
function(a,b){a=this.escapeSpecial(a.innerHTML);var c=this,d='<?xml version="1.0" encoding="utf-8" ?><learnword><text>'+a+"</text></learnword>";$.ajax({type:"POST",url:this.getUrl(),data:d,dataType:"text",error:function(a){c.custom_ajax_error?c.custom_ajax_error(c):rcmail.alert_dialog("An error was encountered on the server. Please try again later.")},success:function(a){}})};this.prepare=function(a,b){this.cnt_errors=this.cnt_errors_fixed=0;this.setStateChanged("checking_spell");this.orginal_text=
"";!b&&this.main_controller&&this.appendIndicator(this.spell_span);this.error_links=[];this.ta_scroll_top=this.text_area.scrollTop;this.ignore=a;var c=$(this.text_area);if(""==c.val()||a)this.custom_no_spelling_error?this.custom_no_spelling_error(this):this.flashNoSpellingErrorState(),this.removeIndicator();else{this.createEditLayer(c.width(),c.height());this.createErrorWindow();$("body").append(this.error_window);try{netscape.security.PrivilegeManager.enablePrivilege("UniversalBrowserRead")}catch(d){}this.main_controller&&
$(this.spell_span).off("click");this.orginal_text=c.val()}};this.parseResult=function(a){var b=/\w+="(\d+|true)"/g,c=/\t/g;a=a.match(/<c[^>]*>[^<]*<\/c>/g);var d=[];if(null==a)return d;for(var e=0,f=a.length;e<f;e++){var k=[];this.errorFound();k.attrs=[];for(var h,g,l=a[e].match(b),m=0;m<l.length;m++)h=l[m].split(/=/),g=h[1].replace(/"/g,""),k.attrs[h[0]]="true"!=g?parseInt(g):g;k.suggestions=[];h=a[e].replace(/<[^>]*>/g,"").split(c);for(g=0;g<h.length;g++)""!=h[g]&&k.suggestions.push(h[g]);d.push(k)}return d};
...
"").off();this.hideErrorWindow()};this.showErrorWindow=function(a,b){this.show_menu_observer&&this.show_menu_observer(this);var c=this,d=$(a).offset(),e=document.createElement("ul");$(this.error_window).html("");$(e).addClass("googie_list toolbarmenu").attr("googie_action_btn","1");var f=this.results[b].suggestions,k=this.results[b].attrs.o,h=this.results[b].attrs.l,g;this.has_dictionary&&!$(a).attr("is_corrected")&&(g=$("<a>").text(this.lang_learn_word).addClass("googie_add_to_dict active"),$("<li>").attr("googie_action_btn",
"1").css("cursor","default").mouseover(c.item_onmouseover).mouseout(c.item_onmouseout).click(function(d){c.learnWord(a,b);c.ignoreError(a,b)}).append(g).appendTo(e));for(var l=0,h=f.length;l<h;l++)g=$("<a>").html(f[l]).addClass("active"),$("<li>").mouseover(this.item_onmouseover).mouseout(this.item_onmouseout).click(function(d){c.correctError(b,a,d.target.firstChild)}).append(g).appendTo(e);if(a.is_changed&&a.innerHTML!=a.old_value){var m=a.old_value;g=$("<a>").addClass("googie_list_revert active").html(this.lang_revert+
" "+m);$("<li>").mouseover(this.item_onmouseover).mouseout(this.item_onmouseout).click(function(d){c.updateOrginalText(k,a.innerHTML,m,b);$(a).removeAttr("is_corrected").css("color","#b91414").html(m);c.hideErrorWindow()}).append(g).appendTo(e)}var f=document.createElement("li"),n=document.createElement("input"),h=document.createElement("button");g=document.createElement("form");l=function(){""!=n.value&&(c.isDefined(a.old_value)||c.saveOldValue(a,a.innerHTML),c.updateOrginalText(k,a.innerHTML,n.value,
b),$(a).attr("is_corrected",!0).css("color","green").text(n.value),c.hideErrorWindow());return!1};$(n).width(120).val($(a).text()).attr("googie_action_btn","1");$(f).css("cursor","default").attr("googie_action_btn","1").on("click",function(){return!1});this.use_ok_pic?$("<img>").attr("src",this.img_dir+"ok.gif").width(32).height(16).css({cursor:"pointer","margin-left":"2px","margin-right":"2px"}).appendTo(h):$(h).text("OK");$(h).addClass("mainaction save googie_ok_button btn-sm").click(l);$(g).attr("googie_action_btn",
"1").css({cursor:"default","white-space":"nowrap"}).submit(l).append(n).append(h).appendTo(f);e.appendChild(f);0<this.extra_menu_items.length&&e.appendChild(this.createListSeparator());var p=function(b){if(b<c.extra_menu_items.length){var d=c.extra_menu_items[b];if(!d[2]||d[2](a,c)){var f=document.createElement("tr"),g=document.createElement("td");$(g).html(d[0]).mouseover(c.item_onmouseover).mouseout(c.item_onmouseout).click(function(){return d[1](a,c)});f.appendChild(g);e.appendChild(f)}p(b+1)}};
p(0);p=null;this.use_close_btn&&e.appendChild(this.createCloseButton(this.hideErrorWindow));this.error_window.appendChild(e);rcmail.triggerEvent("googiespell_create",{obj:this.error_window});g=$(this.error_window).height();f=$(this.error_window).width();l=$(document).height();h=$(document).width();g=d.top+g+20<l?d.top+20:d.top-g;d=d.left+f<h?d.left:d.left-f;0>d&&(d=0);0>g&&(g=0);$(this.error_window).css({top:g+"px",left:d+"px",position:"absolute"}).show();document.all&&!window.opera&&(this.error_window_iframe||
...
f;b?(f=document.createElement("span"),$(f).addClass(b).html(a)):f=document.createTextNode(a);$(e).click(c).mouseover(this.item_onmouseover).mouseout(this.item_onmouseout);e.appendChild(f);d.appendChild(e);return d};this.removeIndicator=function(a){window.rcmail&&rcmail.set_busy(!1,null,this.rc_msg_id)};this.appendIndicator=function(a){window.rcmail&&(this.rc_msg_id=rcmail.set_busy(!0,"checking"))};this.createFocusLink=function(a){var b=document.createElement("a");$(b).attr({href:"javascript:;",name:a});

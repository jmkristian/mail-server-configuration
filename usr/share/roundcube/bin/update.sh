...
if (($confdir = getenv('RCUBE_CONFIG_PATH')) !== null)
    // iniset uses ROUNDCUBE_CONFIG_DIR not RCUBE_CONFIG_PATH,
    // cf. `git grep "getenv.*_CONFIG"`
    putenv('ROUNDCUBE_CONFIG_DIR=' . $confdir);
